const marcoTemporal = new Date(2025, 6, 1, 0, 0, 0);
    ...
    */
    const CANDLE_PICKUP_SOUND_SRC = 'S.M/ColetarVela.mp3';
    ...
    */
    ...
    function drawFire() {
      ...
      const animate = (time = 0) => {
        ...
        if (time - lastTime >= frameDelay) {
          ...
          for (let i = 0; i < 4; i++) fireParticles.push(spawnFlameParticle());
    ...
    function drawSmoke() {
      if (!smokeRunning || !smokeCtx) return;
      ...
      const animate = (time = 0) => {
        ...
        if (time - lastTime >= frameDelay) {
          ...
          for (let i = 0; i < 2; i++) smokeParticles.push(spawnSmoke());
...
/* =========================================================
   MOBILE / TOUCH — PRINCIPAL
   Alternativas para a experiência originalmente pensada para mouse.
========================================================= */
(function(){
  ...
  ['touchstart','touchmove','pointerdown','pointermove'].forEach((evt) => {
    window.addEventListener(evt, (e) => {
      ...
      if (typeof atualizarMouseLuz === 'function') atualizarMouseLuz(p.x, p.y);
    }, { passive:true });
  });

  // Swipe discreto para virar páginas quando o livro está ativo.
  let startX = 0;
  let startY = 0;
  let startT = 0;

  document.addEventListener('touchstart', (e) => {
    if (!bookStage || !bookStage.classList.contains('active')) return;
    if (e.target.closest('button, a, video, input, textarea, #draggable-photo, .draggable-photo, .draggable-video')) return;
    const p = touchPoint(e);
  ...
  document.addEventListener('touchend', (e) => {
    ...
    if (dx < 0 && typeof virarProxima === 'function') virarProxima();
    if (dx > 0 && typeof voltarAnterior === 'function') voltarAnterior();
  }, { passive:true });

  // Garante que vídeos do site não forcem fullscreen no iOS e tentem autoplay quando possível.
